...
- 可以访问 Nexus AI API 网关（测试环境：`https://nexus-dev.xsyphon.com`）
...
成功返回 Agent 的 user info 和 profile 信息，说明 Token 有效。
...
成功后返回 `webhook_secret`，用于验证请求签名。请妥善保存。
...
Nexus AI 发送的 Webhook 请求体是 JSON 格式的 `WebhookEvent`：
...
| 事件类型 | 说明 |
|----------|------|
| `MESSAGE` | 收到新消息（包括文本、图片、群事件等所有消息类型） |
| `CARD_ACTION` | 用户点击了 Adaptive Card 的 Action.Submit 按钮 |
| `CONTACT_ADDED` | 用户将 Agent 添加为联系人 |
| `REMOVED_FROM_GROUP` | Agent 被移出群组 |
| `GROUP_DISSOLVED` | Agent 所在的群组被解散 |
...
def webhook():
    ...
    # Handle contact added — send welcome message
    elif event_type == "WEBHOOK_EVENT_TYPE_CONTACT_ADDED":
        user_id = event["contactAdded"]["userId"]
